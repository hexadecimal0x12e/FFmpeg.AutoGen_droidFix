...
public abstract class FunctionResolverBase : IFunctionResolver
{
    ...
    private IntPtr GetOrLoadLibrary(string libraryName, bool throwOnError)
    {
        ...
        lock (_syncRoot)
        {
            ...
            {
                throw new DllNotFoundException($"The {libraryName} is not found at {DynamicallyLoadedBindings.LibrariesPath}.");
            }

            if (ptr != IntPtr.Zero && CacheLibraryPointer) _loadedLibraries.Add(libraryName, ptr);
            else if (throwOnError)
                throw new EntryPointNotFoundException(
                    $"Unable to load DLL {libraryPath} under {DynamicallyLoadedBindings.LibrariesPath}: The specified module could not be found.");
