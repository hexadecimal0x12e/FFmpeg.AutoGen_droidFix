...
{
    protected override string GetNativeLibraryName(string libraryName, int version, bool _) => $"lib{libraryName}.so"; //.net maui does not support version suffix for android, so we ignore the version parameter
